...
from isaac_arena.policy.data_utils.joints_conversion import remap_policy_joints_to_sim_joints, remap_sim_joints_to_policy_joints
...
class Gr00tClosedloopPolicy(PolicyBase):
    def __init__(
        self, policy_config_yaml_path: Path, num_envs: int = 1, device: str = "cuda"
    ):
        """
    ...
    def get_observations(self, observation: GymSpacesDict) -> dict:
        ...
        joint_pos_state_policy = remap_sim_joints_to_policy_joints(joint_pos_state_sim, self.gr00t_joints_config)

        # Pack inputs to dictionary and run the inference
        policy_observations = {
            "annotation.human.task_description": [self.policy_config.language_instruction],
            "video.ego_view": rgb.reshape(self.num_envs, 1, self.policy_config.target_image_size[0], self.policy_config.target_image_size[1], self.policy_config.target_image_size[2]),
    ...
    def get_action_chunk(self, observation: GymSpacesDict) -> torch.Tensor:
        ...
        robot_action_sim = remap_policy_joints_to_sim_joints(
            robot_action_policy, self.gr00t_joints_config, self.robot_action_joints_config, self.device
        )
